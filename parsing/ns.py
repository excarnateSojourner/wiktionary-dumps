...
def main():
	...
	parser.add_argument('-o', '--output-path-prefix', default='pages_')
	parser.add_argument('-v', '--verbose', action='store_true')
	args = parser.parse_args()
	namespace_groups = []
	for comma_sep in args.namespaces:
		namespace_groups.append([int(ns) for ns in comma_sep.split(',')])
	namespace_filter(args.input_path, namespace_groups, args.output_path_prefix, args.verbose)

def namespace_filter(input_path: str, namespace_groups: list[list[int]], output_path_prefix: str = 'pages_', verbose: bool = False) -> None:
	ns_files = {}
	for group in namespace_groups:
		group_str = ','.join(str(ns) for ns in group)
		group_file = open(f'{output_path_prefix}{group_str}.xml', 'w', encoding='utf-8')
		for ns in group:
